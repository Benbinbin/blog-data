...
* `d3.map(iterable, mapper)` 可迭代对象中的元素均调用映射函数 `mapper`，该函数返回一个值，作为该元素在新数组的「替代值」。该方法一般用于对可迭代对象中（所有元素）进行转换。
...
* **方法 `interval.range(start, stop[, step])`** 返回一个包含一系列 Date 对象的数组，这些 Date 对象在特定的时间范围内，且间隔都是相同的，而且这些 Date 对象是 interval 时间尺度下的**下界值**。
  ...
  :bulb: D3 还为不同的时间尺度下的时距器的方法 `interval.range()` 提供了相应的语法糖 ：
    ...
    * `d3.timeYears(start, stop[, step])` 和 `d3.utcYears(start, stop[, step])` 时间尺度为年级别
    ...
    以上方法中时间的名称为**复数**，这是为了表示这些方法的返回值是一个数组，含有多个 Date 对象
...
:bulb: 以上是构建时间格式器的通用方法，可以获得一个适用于本地语言的时间格式器。D3 为一些常用的语言提供了现成的 `definition` 配置便于直接引用，具体可以参考[这里](https://github.com/d3/d3-time-format/tree/main/locale)
...
* 方法 `locale.parse(specifier)` 和方法 `locale.utcParse(specifier)` 返回一个基于说明符 `specifier` 所指定的格式来将字符串解析为 Date 对象的**解析器**（以下称为 `parser`）。这两种方法中，前者用于解析和构建出地方时的 Date 对象，后者用于解析和构建出 UTC 的 Date 对象。
  ...
  然后可以调用解析器 parser 去解析特定格式的字符串 `parse(str)`，得到相应的 Date 对象或 `null`（如果解析失败）

  :warning: 解析器要求入参 `str` **严格遵循 `specifier` 所指定的格式**，否则无法进行解析。例如说明符 `%Y-%m-%dT%H:%M:%SZ`（这里的 `Z` 是表示字符串最后以 `Z` 结尾，和指令 `%Z` 含义不同）只能用于解析匹配字符串 `"2011-07-01T19:15:28Z"`，而无法解析字符串 `"2011-07-01T19:15:28"`、`"2011-07-01 19:15:28"` 和 `"2011-07-01"`。如果想让解析器更有灵活性，可以匹配多种格式的字符串，可以将多个 parser 封装起来，对同一个 `str` 依次调用，直到返回的不是 `null` 结果为止

  :bulb: 在解析器所使用的说明符中，指令 `%d` 和 `%e` 作用相同，都表示/匹配月份的日期

  :bulb: 如果时间使用英文表示，则可以使用 D3 提供的一些方法，直接构建出解析器，它们是时间格式器的**语法糖**（不需要先使用方法 `d3.timeFormatLocale(definition)` 获取时间格式器，再使用 `locale.parse()` 或 `locale.utcParse()` 构建出解析器）

    * `d3.timeParse(specifier)` 等价于 `locale.parser()` 方法，使用默认配置
    * `d3.utcParse(specifier)` 等价于 `locale.utcParse()` 方法，使用默认配置

  :bulb: 如果字符串所表示的时间使用 [ISO 8601 标准](https://en.wikipedia.org/wiki/ISO_8601)，可以使用 `d3.isoParse` 获取一个解析器。如果希望构建一个严格遵循 ISO 8601 标准的 UTC 时间解析器，应该使用以下方法

  ```js
  const strictIsoParse = d3.utcParse("%Y-%m-%dT%H:%M:%S.%LZ");
  ```

* 方法 `locale.format(specifier)` 和方法 `locale.utcFormat(specifier)` 返回一个将 Date 对象格式化为特定的字符串的**格式器**（以下称为 `formatter`）。

  然后可以调用格式器 formatter 将 Date 对象格式化为特定的字符串 `formatter(dateObj)`
  ...
  :bulb: 如果时间使用英文表示，则可以使用 D3 提供的一些方法，直接构建出格式器，它们是时间格式器的**语法糖**

    * `d3.timeFormat(specifier)` 等价于 `locale.format()` 方法，使用默认配置
    * `d3.utcFormat(specifier)` 等价于 `locale.utcFormat()` 方法，使用默认配置

  :bulb: 如果希望将 Date 对象格式化为符合 ISO 8601 标准的字符串，可以使用 `d3.isoFormat` 获取一个格式器，它会在内部优先调用 JavaScript 原生的 [`dateObj.toISOString()` 方法](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Date/toISOString)实现转换

以上方法 `d3.timeFormatLocale(definition)` 可以构建出一个时间格式器 locale，但是对于 D3 内置提供的一些快速构建解释器和构造器的方法（语法糖） `d3.timeFormat()`、`d3.timeParse()`、`d3.utcFormat()` 和 `d3.utcParse()` 并没有修改配置，还是以默认配置(即英文)来解析和格式化时间。如果希望同时修改默认配置，可以使用方法 `d3.timeFormatDefaultLocale(definition)` 它返回一个时间格式器，同时会覆盖内置方法 `d3.timeFormat()`、`d3.timeParse()`、`d3.utcFormat()` 和 `d3.utcParse()` 的默认配置也会一并修改，让模块使用体验更好。

```js
d3.json(
  "https://cdn.jsdelivr.net/npm/d3-time-format@3.0.0/locale/zh-CN.json"
).then((cnDefinition) => {
  const time = new Date();

  // 使用中文配置来生成一个时间格式器
  const locale = d3.timeFormatLocale(cnDefinition);
  // 通过时间格式器创建的 formatter 使用中文配置
  const formatter = locale.format("%c");
  console.log(formatter(time)); // 2022年2月7日 星期一 11:21:39
  // 使用语法糖创建的 formatter 依然使用默认（即英文）配置
  const sugarFormatter = d3.timeFormat("%c");
  console.log(sugarFormatter(time)); // 2/7/2022, 11:21:39 AM
});
```
...
d3.json(
  ...
  const time = new Date();
  // 使用中文配置来生成一个时间格式器，同时修改了默认配置
  const changerDefaultLocale = d3.timeFormatDefaultLocale(cnDefinition);
  const changeDefaultLocaleFormatter = changerDefaultLocale.format("%c");
  console.log(changeDefaultLocaleFormatter(time)); // 2022年2月7日 星期一 11:25:10
  // 即使使用简便的语法糖来获取一个格式器，也是使用中文配置的
  const sugarFormatter = d3.timeFormat("%c");
  console.log(sugarFormatter(time)); // 2022年2月7日 星期一 11:25:10
});
```
